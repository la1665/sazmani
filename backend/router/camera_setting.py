...
async def api_create_setting(setting: CameraSettingCreate, db: AsyncSession = Depends(get_db), current_user: UserInDB=Depends(get_admin_user)):
...
async def api_update_setting(setting_id: int, setting: CameraSettingUpdate, db: AsyncSession = Depends(get_db), current_user: UserInDB=Depends(get_admin_user)):
    setting_op = CameraSettingOperation(db)
