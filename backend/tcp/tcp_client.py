...
class SimpleTCPClient(basic.LineReceiver):
    ...
    async def _fetch_and_send_lpr_settings(self):
        try:
            print(self.factory.lpr_id)
            # lpr_settings = reactor.callInThread(fetch_lpr_settings(self.factory.lpr_id))
            # lpr_settings = await ensureDeferred(fetch_lpr_settings(self.factory.lpr_id))
            lpr_settings = await reactor.callInThread(lambda: ensureDeferred(fetch_lpr_settings(self.factory.lpr_id)))
    ...
    #             # lpr_settings = await fetch_lpr_settings(self.factory.lpr_id)
    #             # lpr_settings = yield ensureDeferred(fetch_lpr_settings(self.factory.lpr_id))
    #             lpr_settings = reactor.callWhenRunning(lambda: ensureDeferred(fetch_lpr_settings(self.factory.lpr_id)))
    #             print(" lpr settings fetched...")
    #             hmac_key = settings.HMAC_SECRET_KEY.encode()
    #             data_str = json.dumps(lpr_settings, separators=(',', ':'), sort_keys=True)
    #             hmac_signature = hmac.new(hmac_key, data_str.encode(), hashlib.sha256).hexdigest()
    #             settings_message = {
