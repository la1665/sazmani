...
class KeyOperation(CrudOperation):
    ...
    async def get_keys_by_relay_id(self, relay_id: int):
        ...
        """
        result = await self.db_session.execute(select(DBRelayKey).filter(DBRelayKey.relay_id == relay_id).order_by(DBRelayKey.created_at.desc()))
    ...
    async def get_keys_by_camera_id(self, camera_id: int):
        ...
        """
        result = await self.db_session.execute(select(DBRelayKey).filter(DBRelayKey.camera_id == camera_id).order_by(DBRelayKey.created_at.desc()))
        return result.scalars().all()
